...
public class TitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    public TitleItemDecoration(Context context, ArrayList<User> list){
        super();
        this.mDatas = list;
        mPaint = new Paint();
        mBounds = new Rect();
        ...
        for (int i = 0;i < childCount; i++){
            ...
            if (position > -1){
                if (position == 0){//等于0时有title
                    ...
                    if (mDatas.get(position).getOrgan() != null
                            && !mDatas.get(position).getOrgan().toString().equals(mDatas.get(position - 1).getOrgan())){
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position){
        ...
        mPaint.getTextBounds(mDatas.get(position).getOrgan(), 0, mDatas.get(position).getOrgan().length(), mBounds);
        c.drawText(mDatas.get(position).getOrgan(),child.getPaddingLeft(),
...
//        super.onDrawOver(c, parent, state);
        ...
        String   tag = mDatas.get(position).getOrgan();
        ...
        if ((position + 1) < mDatas.size()){//当前第一个可见的Item的tag,不等于以后一个Item的tag,说明悬浮的View需要切换了
            if (null != tag && ! tag.equals(mDatas.get(position + 1).getOrgan())){
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        if (position > -1) {// RecycleView的item position在重置时可能为-1，保险起见加判断
            if(position == 0){//等于0肯定存在title
                ...
                if(mDatas.get(position).getOrgan() != null && !mDatas.get(position).getOrgan().toString().equals(mDatas.get(position - 1).getOrgan())){
                    outRect.set(0,mTitleHeight, 0,0);
                } else {
                    outRect.set(0, 0, 0, 0);
                }
