...
public class CheckTitleItemDecoration extends RecyclerView.ItemDecoration {
    ...
    private static final String CHECK_IN = "已签到的";
    private static final String NO_CHECK_IN = "未签到的";
    private String mNoCheckName;
    ...
    public CheckTitleItemDecoration(Context context, ArrayList<User> list,int divider){
        super();
        this.mDatas = list;
        mdivider = divider;
        mPaint = new Paint();
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position){
        ...
        if (position == mdivider){
            mPaint.getTextBounds(CHECK_IN, 0, CHECK_IN.length(), mBounds);
            c.drawText(CHECK_IN,child.getPaddingLeft(),
                    child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2),mPaint);
        } else {
            mPaint.getTextBounds(NO_CHECK_IN, 0, NO_CHECK_IN.length(), mBounds);
            c.drawText(NO_CHECK_IN,child.getPaddingLeft(),
                    child.getTop() - params.topMargin - (mTitleHeight / 2 - mBounds.height() / 2),mPaint);
        }
